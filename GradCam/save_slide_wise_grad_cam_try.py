...
def save_slidewise_gradcam(patient,visualization_fp,model,infer_on):
    ...
    im_slide = np.ones(((W//4)+512,(H//4)+512,3),dtype=np.uint8)*240
    ...
    for x in range(0,W//4,patch_size):
        for y in range(0,H//4,patch_size):
            ...
            im_slide[x:x+patch_arr.shape[0],y:y+patch_size,:] = patch_arr
    im_slide_gt= im_slide.copy()
    for k in glob.glob('{}/{}_{}/*{}*'.format(visualization_fp,model,infer_on,patient)):
        if '_Cam_On_Image.png' in k:
            original_patch = Image.open('{}/{}.png'.format(patch_fp[0],k.split('/')[-1].split('_Cam_')[0])) .convert('RGB')
            
            original_patch = original_patch.resize((224,224))
            original_patch = original_patch.resize((patch_size,patch_size))
            original_patch_arr = np.swapaxes(np.array(original_patch),1,0)

            cam_patch = Image.open(k).convert('RGB')
            cam_patch = cam_patch.resize((patch_size,patch_size))
            cam_patch_arr = np.swapaxes(np.array(cam_patch),1,0)

            cam_patch_heatmap = Image.open(k.replace('Cam_On_Image','Cam_thresholded')).convert('RGB')
            cam_patch_heatmap = cam_patch_heatmap.resize((patch_size,patch_size))
            cam_patch_heatmap_arr = np.swapaxes(np.array(cam_patch_heatmap),1,0)

            x = int(k.split('_X_')[-1].split('_Y')[0])
            y = int(k.split('_X_')[-1].split('_Y_')[-1].split('_')[0])

            patch_to_save = original_patch_arr.copy()
            patch_to_save[np.where(cam_patch_heatmap_arr==255)] = cam_patch_arr[np.where(cam_patch_heatmap_arr==255)]

            im_slide[x:x+patch_size,y:y+patch_size,:] = patch_to_save

    for l in glob.glob('{}/{}_{}/*{}*'.format(visualization_fp,infer_on,infer_on,patient)):
        if '_Cam_On_Image.png' in l:
            original_patch = Image.open('{}/{}.png'.format(patch_fp[0],l.split('/')[-1].split('_Cam_')[0])) .convert('RGB')
            ...
            cam_patch_heatmap = Image.open(l.replace('Cam_On_Image','Cam_thresholded')).convert('RGB')
            cam_patch_heatmap = cam_patch_heatmap.resize((patch_size,patch_size))
            cam_patch_heatmap_arr = np.swapaxes(np.array(cam_patch_heatmap),1,0)
            ...
            im_slide_gt[x:x+patch_size,y:y+patch_size,:] = patch_to_save
